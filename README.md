...
Olivaw is a local-first personal assistant framework. The v0.8.0 release
expands the original deterministic briefing foundation into an assistant-first
local web experience with source-grounded conversation, explicit knowledge
modes, weather context, local model diagnostics, action routing, and bounded
...
OLIVAW_WEATHER_ENABLED=false
OLIVAW_WEATHER_LATITUDE=
OLIVAW_WEATHER_LONGITUDE=
OLIVAW_WEATHER_LOCATION_NAME=
OLIVAW_WEATHER_UNITS=fahrenheit
```
...
`olivaw chat` routes prompts through source-backed, derived, deterministic, or
model-knowledge paths depending on the request. It remains conservative in v0:
weather and network prompts use registered sources when available, operational
unknowns are labeled as unknown instead of guessed, and general knowledge uses
the configured provider without claiming source access.
...
Responses can carry explicit internal attribution states:
...
- Derived: the answer is deterministic synthesis over source-backed state.
- Model knowledge: the answer is explanation or general knowledge generated by
  the selected provider without claiming source access.
- Unknown operational state: the request asks about local state that no
  registered source currently provides.
...
integrations that are not present. For example, a weather request uses
WeatherSource when weather is configured and otherwise returns a friendly
source-unavailable message instead of asking a model to guess. Questions like
"Explain local-first architecture" still route to the configured model provider
as model-knowledge responses.

Model providers are not tools. OpenAI or Ollama can produce text, but they do
not give Olivaw web search, calendar, email, news, sports, or stock-price
access. Weather access is provided only through configured WeatherSource data,
not through model claims.
...
PrimeObserverSource, CoreSignalSource, and WeatherSource. Future source-backed
answers can cite sources such as CalendarSource and EmailSource once those
...
- `/` shows the assistant-first landing experience, ambient presence, prompt
  suggestions, and relevant context capsules.
- `/chat` provides the conversation surface with source-aware attribution and
  contextual follow-up cards.
...
health status, and `fetch()` method. Registered sources are also normalized into
an aggregate source view for facts, interpretation items, actions, references,
diagnostics, and Health Review context.

v0.8.0 includes five sources:
...
WeatherSource is optional and disabled until configured. When enabled with
latitude and longitude, it fetches current conditions and a one-day forecast from
Open-Meteo, normalizes weather facts into the source aggregate, and lets chat and
briefing surfaces answer weather prompts with source-backed attribution. Without
configuration, weather requests degrade to clear source-unavailable guidance
instead of model guesses.

CalendarSource and EmailSource are roadmap items only. They are not integrated
yet.
...
It currently uses ManualSource, FileSource, PrimeObserverSource,
CoreSignalSource, and optional WeatherSource. The output includes source status,
source-backed highlights, file previews, weather context when configured, a
current-state Prime Observer section, an interpretation-focused Core Signal
section, event-aware Core Signal findings when emitted, source notes, and
attribution such as:
...
- Assistant: moved the web experience to an assistant-first landing and
  conversation flow, added ambient presence states, refined conversation
  timelines, and kept contextual cards tied to relevant prompts and source
  state.
- Trust: added source-grounded chat paths, explicit knowledge-mode attribution,
  provenance integrity checks, and leakage prevention so model-knowledge answers
  do not imply access to registered sources.
- Performance: added local model benchmarking, Ollama request tuning, and fast
  deterministic paths for source-backed weather, network, operational, and
  action-suggestion prompts.
- Architecture: added a balanced cloud fallback policy, clearer routing between
  source-backed, derived, unknown, unavailable, local-model, and cloud-eligible
  answers, and improved action request handling.
- Sources and operations: added optional WeatherSource integration, source
  aggregation for facts and diagnostics, Health Review improvements, and direct
  LaunchAgent restart helpers for the local web UI.

### v0.7.0

The v0.7.0 release established source-backed briefing behavior, Prime Observer
and Core Signal presentation boundaries, provider routing, and release-ready
local web/CLI validation surfaces. Historical release behavior remains documented
in the repository history and tags.
